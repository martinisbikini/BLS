...
frq_offset = np.array([-0.625,-0.5,-0.625])[:,None] #bls spectrum offset to RF frequ.
...
left = np.max(daq)-np.min(daq)+0.0
right = 0.01
...
plt.figure(num=52)
plt.clf()
plt.plot(x1,Gint[0,:]*left,linewidth=2,color='red') 
plt.xlabel('Distance ($\mu$m)', fontsize=14)
plt.ylabel('DAQ (V)', fontsize=14)
...
plt.xlabel('Distance ($\mu$m)', fontsize=14)
plt.ylabel('Relative Photon cts', fontsize=14)
